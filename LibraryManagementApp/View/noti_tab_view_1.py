...
class Delete(): # To confirm delection
    def __init__(self, root, delete_type):
        ...
        #Title Label
        Label(self.delete_noti, text = delete_map[delete_type][0], font = ('Montserrat', 18, 'bold'), bg = 'white', fg = 'black').pack(pady = (20,10))
        #Message Label
        Label(self.delete_noti, text = delete_map[delete_type][1], font = ("Montserrat", 12), bg = 'white', fg = 'black').pack(pady = (10,10))

        # Yes Button
        Button(self.delete_noti, width = 13, text = "Yes", highlightbackground = 'white', highlighthickness = 1, command = lambda: self.choice('yes')).pack(pady = 10)
        # No Button
...
class Message_1(): # To notify message (Type 1: When clicked 'OK' button -> messagebox disappear)
    def __init__(self, root, delete_type):
        ...
        #Tile Label
        Label(self.message_1, text = message_1_map[delete_type], font = ("Montserrat", 18, 'bold'), bg = 'white', fg = 'black').pack(pady = (20,10))

        # OK Button
        Button(self.message_1, width = 13, text = "OK", highlightbackground= 'white', highlightthickess = 1, command = self.message_1.destroy).pack(pady = 10)
...
class Message_2(): # To notify message (Type 2: when clicked 'Return' button -> switch back to the first frame of sub-function)
    def __init__(self, root, message_type):
        ...
        # Title Label
        Label(self.message_2, text = message_2_map[message_type], font = ("Montserrat", 18, 'bold'), bg = 'white', fg = 'black').pack(pady = (20,10))

        # Return Button
        Button(self.message_2, width = 13, text = "Return", highlightbackground= 'white', highlightthickess = 1, command = lambda: self.back_to_subfun()).pack(pady = 10)
...
class Invalid(): # To notify Invalid input
    def __init__(self, root, invalid_type):
        ...
        invalid_map = {'account': ("Invalid Account", "Please double-check your email and password!"),
                       'quantity': ('Invalid Quantities', "No. of books borrow need to be larger than zero and lower or equal to Availible quantities")}
        
        # Title Label
        Label(self.invalid, text = invalid_map[invalid_type][0], font = ("Montserrat", 18, 'bold'), bg = 'white', fg = 'black').pack(pady = (20,10))
        # Message Label
        Label(self.invalid, text = invalid_map[invalid_type][1], font = ("Montserrat", 12), bg = 'white', fg = 'black').pack(pady = (10,10))

        # Ok button
        Button(self.invalid, width = 13, text = "OK", highlightbackground= 'white', highlightthickess = 1, command = lambda: self.invalid.destroy).pack(pady = 10)
...
class Drop_Off():
    def __init__(self, root, receipt_status):
        ...
        #Title Label
        Label(self.delete_noti, text = "Drop Off Successfully!", font = ('Montserrat', 18, 'bold'), bg = 'white', fg = 'black').pack(pady = (20,10))
